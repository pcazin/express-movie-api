...
export default class ActorRepository {
    ...
    async getById(id: number): Promise<actors | Error> {
        try {
            const actor: actors | null = await prisma.actors.findUnique({
                where: { id: id },
            });
    ...
    async getByName(firtName: string, lastName: string): Promise<actors | Error> {
        try {
            const actor: actors | null = await prisma.actors.findFirst({
                where: {
                    first_name: firtName,
                    last_name: lastName
                },
    ...
    async create(actor: ActorPayload): Promise<actors | Error> {
        try {
            const newActor: actors = await prisma.actors.create({
                data: actor,
            });
            return newActor;
        } catch (err) {
            return new Error(
                "Error: create() method in ActorRepository failed."
            );
        }
    }
